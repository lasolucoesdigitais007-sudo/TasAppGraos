...
export default function App() {
        ...
        if (snapshot.empty) {
          ...
          const list = snapshot.docs.map((doc) => {
            ...
            return {
              ...
              promoPrice:
                d.promoPrice !== undefined ? Number(d.promoPrice) : null,
  ...
  const [showAdminTab, setShowAdminTab] = useState<
    "products" | "categories" | "recipes" | "articles" | "json_io" | "finance"
  >("products");
  ...
  const [orderPaymentMethod, setOrderPaymentMethod] = useState<
    "PIX" | "Cartão de Crédito" | "WhatsApp Direct"
  >("PIX");
  ...
  const [selectedArticle, setSelectedArticle] =
    useState<EducationalArticle | null>(null);
  ...
  const [activeProfileTab, setActiveProfileTab] = useState<
    "perfil" | "history" | "settings" | "logout"
  >("perfil");
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message:
        "Use o cupom BEMESTAR10 para receber 10% OFF no seu primeiro pedido do zero! 🌱",
      date: "Hoje",
      read: false,
    },
  ]);
  ...
  const DEFAULT_BANNERS = [
    {
      title: "Alimentação Ativa & Consciente",
      desc: "Sinta os benefícios dos grãos cultivados com pureza e amor.",
      tag: "100% Orgânico",
      image: HERO_IMAGE_PATH,
    },
    {
      title: "Qualidade no Seu Tempo",
      desc: "Compre a granel de forma prática nas embalagens ideais para o seu lar.",
      tag: "A Granel Premium",
      image: TEA_IMAGE_PATH,
    },
    {
      title: "Chás Medicinais Colhidos à Mão",
      desc: "Experiência sensorial requintada para resgatar sua clareza de foco.",
      tag: "Blend de Outono",
      image: TEA_IMAGE_PATH,
    },
  ];
  ...
  const handleAddProductToCart = (product: Product, weightGrams: number) => {
    ...
    const price = Number(
      ((product.pricePer100g / 100) * weightGrams).toFixed(2),
    );
    ...
    setCart((prev) => {
      ...
      if (existing) {
        return prev.map((item) =>
          item.id === cartLineId
            ? {
                ...item,
                weightGrams: item.weightGrams + weightGrams,
                totalPrice: Number((item.totalPrice + price).toFixed(2)),
              }
            : item,
        );
      }
      return [
        ...prev,
        { id: cartLineId, product, weightGrams, totalPrice: price },
      ];
    });
  };

  const handleAddProductToCartByProductId = (
    productId: string,
    weightGrams: number,
  ) => {
      ...
      prev.map((item) => {
        if (item.id === itemId) {
          ...
          const newPrice = Number(
            ((item.product.pricePer100g / 100) * newWeight).toFixed(2),
          );
  ...
  const handleApplyCoupon = () => {
    ...
    if (formatted === "TAS15") {
      ...
      alert(
        "Cupom de desconto BEMESTAR10 aplicado! Você recebeu 10% OFF! 🌱🏷️",
      );
    } else {
  ...
  const shippingFee = cartSubtotal > 100 ? 0 : 12.9;
  const cartTotal = Number(
    (cartSubtotal - discountAmount + shippingFee).toFixed(2),
  );
  ...
  const getWhatsAppMessageLink = () => {
    const itemsList = cart
      .map(
        (item) =>
          `• ${item.product.name} (${item.weightGrams}g) - R$ ${item.totalPrice.toFixed(2)}`,
      )
      .join("%0A");
    const payment =
  ...
  const handlePlaceOrder = () => {
    ...
    const orderWithCustomer = {
      ...
      customerName:
        user.name || auth.currentUser?.displayName || "Cliente Anonimizado",
      customerEmail:
        user.email || auth.currentUser?.email || "anonimo@tasgraos.com",
              ...
              >
                ...
                <div className="flex flex-col items-center">
                  ...
                  >
                    ...
                    <div className="text-tas-olive font-serif text-3xl font-extrabold tracking-tighter">
                      TG
                    </div>
                  </motion.div>
                  <motion.h1
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-[10px] text-tas-bege/50 tracking-wide">
                    Moagem artesanal a granel localizada em SP
                  </span>
                </div>
              </motion.div>
            )}
            ...
            {currentView === ActiveView.LOGIN && (
              ...
              >
                ...
                <div className="relative flex justify-center items-center pt-2 z-10 w-full max-w-sm mx-auto">
                  <span className="text-xs font-serif font-bold text-tas-dark/40 uppercase tracking-widest">
                    Portal de Acesso
                  </span>
                </div>
                ...
                <div className="relative my-auto py-6 space-y-6 z-10 max-w-sm mx-auto w-full">
                  ...
                  <div className="text-center space-y-2 flex flex-col items-center">
                    ...
                    />
                    <p className="text-xs text-tas-terroir opacity-80 font-medium">
                      Sua Dose Diária de Saúde e Vitalidade
                    </p>
                  </div>
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-tas-bege/80 shadow-3xs space-y-4">
                    ...
                    {isGestorPortal ? (
                      ...
                      <div className="space-y-3.5">
                        <div className="bg-amber-50/50 p-2.5 rounded-xl border border-[#D4AF37]/30 text-center text-[10px] text-tas-dark/80 font-medium leading-relaxed">
                          🔒 Portal exclusivo para o gestor cadastrar, editar e
                          excluir produtos, e criar promoções da{" "}
                          <strong>Tas Grãos</strong>.
                        </div>

                        <div>
                          <label className="text-[10px] uppercase font-bold text-tas-terroir tracking-wider block mb-1 text-left">
                            E-mail do Gestor
                          </label>
                        ...
                        <button
                          onClick={async () => {
                            ...
                            try {
                              ...
                              alert(
                                "Seja muito bem-vindo, Gestor Tas Grãos! 🛠️🌱",
                              );
                              setCurrentView(ActiveView.HOME);
                            } catch (error: any) {
                              if (error.code !== "auth/operation-not-allowed") {
                                console.error(
                                  "Firebase Gestor Login Fail:",
                                  error,
                                );
                              }
                              ...
                              if (
                                ...
                                alert(
                                  "Acesso como Gestor concedido em Modo de Demonstração Local! 🌱⚙️",
                                );
                                setCurrentView(ActiveView.HOME);
                              } else {
                                alert(
                                  "Erro de login ou senha de gestor incorreta! Verifique as credenciais ou utilize a senha de desenvolvimento na versão de testes.",
                                );
                              }
                            } finally {
                        ...
                        <button
                          onClick={async () => {
                            ...
                            if (
                              ...
                              alert(
                                "Este e-mail é de uso exclusivo do Gestor. Por favor, utilize a aba Área do Gestor para realizar o login!",
                              );
                              return;
                            }
                            setIsGoogleLoading(true);
                            try {
                              ...
                              alert(
                                "Nota: Acessando via modo demonstrativo local para os seus testes! Bem-vindo(a) ao Tas Grãos! 🌱",
                              );
                              setCurrentView(ActiveView.HOME);
                            } finally {
                              setIsGoogleLoading(false);
                        ...
                        <button
                          onClick={async () => {
                            if (!registerName.trim()) {
                              alert(
                                "Por favor, preencha o campo Nome Completo.",
                              );
                              return;
                            }
                            ...
                            if (
                              ...
                              alert(
                                "Este e-mail é de uso exclusivo do Gestor Oficial da Tas Grãos. Por favor, acesse a Conta de Gestor pelo Portal de Entrada!",
                              );
                              return;
                            }
                            if (
                              ...
                              alert(
                                "Por questão de segurança do Firebase, a senha deve conter pelo menos 6 caracteres.",
                              );
                              return;
                            }
                            setIsGoogleLoading(true);
                            try {
                              ...
                              alert(
                                `Cadastrado(a) com sucesso, ${registerName}! 🌱`,
                              );
                              setCurrentView(ActiveView.HOME);
                            } catch (error: any) {
                              if (error.code !== "auth/operation-not-allowed") {
                                console.error(
                                  "Firebase Auth Register Error:",
                                  error,
                                );
                              }
                              ...
                              alert(
                                `Bem-vindo(a), ${registerName}! Sua conta foi ativada em Modo Local para teste imediato de todas as funções do app! 🌱`,
                              );
                              setCurrentView(ActiveView.HOME);
                            } finally {
                              setIsGoogleLoading(false);
                    ...
                    <div className="relative flex items-center justify-center py-1">
                      ...
                      <span className="relative bg-white px-3 text-[10px] font-bold text-tas-terroir uppercase">
                        ou
                      </span>
                    </div>
                ...
                {isGoogleLoading && (
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-6 rounded-2xl border border-tas-bege w-full max-w-xs text-center space-y-4 shadow-xl select-none animate-slideup">
                      ...
                      <div>
                        <h4 className="font-serif text-sm font-bold text-tas-dark">
                          Fazendo login
                        </h4>
                        <p className="text-[10px] text-tas-terroir mt-1">
                ...
                >
                        ...
                        {currentView === ActiveView.HOME ? (
                          ...
                          <h1 className="font-serif text-base text-tas-dark font-bold tracking-tight uppercase">
                            {currentView === ActiveView.CATEGORIES &&
                              "Produtos Premium"}
                            {currentView === ActiveView.PRODUCT_DETAIL &&
                              "Detalhes do Grão"}
                            {currentView === ActiveView.CART &&
                              "Carrinho de Saúde"}
                            {currentView === ActiveView.CHECKOUT &&
                              "Finalizando Pedido"}
                    ...
                    {showNotifications && (
                      ...
                      >
                        <div className="flex justify-between items-center pb-2 border-b border-tas-bege mb-2">
                          <span className="text-xs font-bold text-tas-dark tracking-wide uppercase">
                        ...
                        <div className="space-y-2.5">
                          {notifications.map((n) => (
                            <div
                              key={n.id}
                              className={`p-2.5 rounded-xl text-left border ${n.read ? "bg-white border-gray-100" : "bg-tas-bege/20 border-tas-gold/15"}`}
                            >
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-bold text-tas-dark">
                                  {n.title}
                                </span>
                              ...
                              <p className="text-[11px] text-tas-dark/85 mt-0.5 leading-tight">
                                {n.message}
                              </p>
                            </div>
                          ))}
                  ...
                  <main className="flex-1 overflow-y-auto app-scrollbar p-4 space-y-6">
                      ...
                      >
                        ...
                        <div className="bg-white p-3.5 rounded-2xl border border-tas-bege/40 shadow-sm flex items-center gap-2">
                          ...
                          {searchQuery && (
                            <button
                              ...
                              className="text-xs text-gray-400 font-bold px-1.5"
                            >
                              ✕
                            </button>
                          )}
                        ...
                        <div className="relative rounded-3xl overflow-hidden aspect-[16/9] shadow-md border border-tas-bege/80 bg-tas-dark select-none">
                          {activeBanners.length > 0 && (
                            <>
                              {(() => {
                                const safeIndex =
                                  currentPromoIndex % activeBanners.length;
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-3">
                            <h2 className="font-serif text-base tracking-wide text-tas-dark font-black">
                              Categorias Seletas
                            </h2>
                            <button
                              onClick={() => {
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-3">
                            ...
                            <span className="text-[10px] bg-tas-bege/80 text-tas-terroir font-bold px-2 py-0.5 rounded-md uppercase tracking-wide">
                              Selecionado À Mão
                            </span>
                          </div>
                              ...
                              return (
                                ...
                                >
                                  <div className="relative rounded-2xl overflow-hidden aspect-square mb-2.5 bg-tas-cream">
                                    ...
                                    >
                                      <Heart
                                        className={`h-3.5 w-3.5 ${isFav ? "fill-red-500 text-red-500" : "text-gray-400"}`}
                                      />
                                    </button>
                                    {user && (
                                    ...
                                    {p.isVegan && (
                                      <span className="absolute bottom-2 left-2 text-[8px] font-bold bg-emerald-600 text-white px-2 py-0.5 rounded-full uppercase tracking-wider">
                                        Vegan
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  <div
                                    onClick={() => {
                                      ...
                                      setCurrentView(ActiveView.PRODUCT_DETAIL);
                                    }}
                                    className="text-left cursor-pointer"
                                  >
                                    <h3 className="text-xs font-bold text-tas-dark line-clamp-1 hover:text-tas-gold">
                                      {p.name}
                                    </h3>
                                    <p className="text-[10px] text-tas-terroir">
                                      A partir de
                                    </p>
                                    <div className="flex justify-between items-center mt-1">
                                      <span className="text-xs sm:text-sm font-black text-tas-olive font-mono">
                                        R$ {p.pricePer100g.toFixed(2)}
                                        <span className="text-[9px] font-normal text-tas-dark">
                                          /100g
                                        </span>
                                      </span>
                                      <button
                                        onClick={(e) => {
                                          ...
                                          alert(
                                            `${p.name} (250g) adicionado ao carrinho! 🛒`,
                                          );
                                        }}
                        ...
                        <div className="bg-tas-dark text-white rounded-[2rem] p-5 border border-tas-gold/15 shadow-md relative overflow-hidden select-none">
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[9px] uppercase font-mono tracking-widest font-black text-tas-gold">
                              Fatos & Sabedoria de @tasgraos
                            </span>
                          </div>
                          <h3 className="font-serif text-lg font-bold text-white mb-2 leading-tight">
                            O que você consome te define.
                          </h3>
                          <p className="text-xs text-tas-bege/80 leading-relaxed mb-4">
                            Cada grão, erva e semente carrega uma medicina viva
                            formulada pela terra. Explore a nossa central
                            educativa baseada em saúde integrativa.
                          </p>
                          <button
                            onClick={() =>
                              setCurrentView(ActiveView.BENEFICIOS)
                            }
                        ...
                        <div className="border border-tas-sand bg-white rounded-2xl p-4 flex gap-3 items-center text-left">
                          <div className="h-8.5 w-8.5 rounded-full bg-tas-bege flex items-center justify-center font-bold text-xs shrink-0 text-tas-olive">
                            ✨
                          </div>
                          <div>
                            <p className="text-[10px] text-gray-400 font-bold uppercase">
                              Qualidade Tas Garantida
                            </p>
                            <p className="text-xs text-tas-dark leading-tight">
                              Embalado e selado hermeticamente sob rigorosa
                              vigilância de frescor e sabor.
                            </p>
                          </div>
                        </div>
                      </motion.div>
                    ...
                    {currentView === ActiveView.CATEGORIES && (
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="space-y-4 pb-20 text-blue-900"
                      >
                        ...
                        <div className="flex justify-between items-center text-xs text-tas-terroir pt-1 font-semibold">
                          <span>
                            Exibindo {filteredProducts.length} produtos em "
                            {selectedCategory}"
                          </span>
                          {user && (
                            <button
                        ...
                        {filteredProducts.length === 0 ? (
                          <div className="text-center py-12 space-y-2">
                            <p className="text-sm font-semibold text-tas-dark">
                              Nenhum grão ou produto encontrado.
                            </p>
                            <p className="text-xs text-tas-terroir">
                              Experimente alterar os termos de busca ou filtros.
                            </p>
                          </div>
                        ) : (
                              ...
                              return (
                                ...
                                >
                                  <div className="relative rounded-2xl overflow-hidden aspect-square mb-2.5 bg-tas-cream">
                                    ...
                                    {p.isGlutenFree && (
                                      <span className="absolute bottom-2 left-2 text-[8px] font-bold bg-tas-olive text-white px-2 py-0.5 rounded-full uppercase tracking-wider">
                                        Glúten Free
                                      </span>
                                    )}
                                  </div>
                    ...
                    {currentView === ActiveView.PRODUCT_DETAIL &&
                      selectedProduct && (
                        <motion.div
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          className="space-y-5 pb-20 text-left"
                        >
                          ...
                          <div className="relative rounded-[2rem] overflow-hidden aspect-square shadow-md bg-white border border-tas-bege/70">
                            ...
                            >
                              <Heart
                                className={`h-4.5 w-4.5 ${favorites.includes(selectedProduct.id) ? "fill-red-500 text-red-500" : "text-gray-400"}`}
                              />
                            </button>
                          </div>
                          ...
                          <div>
                            ...
                            <h2 className="font-serif text-xl sm:text-2xl font-bold text-tas-dark leading-tight">
                            ...
                            <div className="flex items-center gap-1.5 mt-1 text-sm font-semibold">
                              ...
                              <span className="text-tas-dark font-mono mt-0.5">
                                {selectedProduct.rating}
                              </span>
                              <span className="text-xs text-tas-terroir font-normal">
                                ({selectedProduct.reviews.length} avaliações do
                                Instagram)
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="bg-white p-4 rounded-3xl border border-tas-bege/60 shadow-xs space-y-4">
                            <div className="flex justify-between items-center">
                              <span className="text-xs font-bold text-tas-dark uppercase tracking-wide">
                                Comprar a Granel
                              </span>
                              <span className="text-xs text-tas-terroir">
                                Preço proporcional
                              </span>
                            </div>
                            ...
                            <div className="flex justify-between items-center pt-2 border-t border-tas-bege/70">
                              <div>
                                <p className="text-[10px] text-tas-terroir uppercase font-bold tracking-wider">
                                  Valor do seu Peso selecionado
                                </p>
                                <p className="text-2xl font-mono text-tas-olive font-black tracking-tight">
                                  R${" "}
                                  {(
                                    (selectedProduct.pricePer100g / 100) *
                                    selectedWeightGrams
                                  ).toFixed(2)}
                                </p>
                              </div>
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  alert(
                                    `${selectedWeightGrams}g de ${selectedProduct.name} incluídos com sucesso no carrinho! 🌱🛒`,
                                  );
                                }}
                          ...
                          <div className="space-y-2">
                            <h3 className="font-serif text-sm font-bold text-tas-dark uppercase tracking-wide">
                              Descrição do Produto
                            </h3>
                            <p className="text-xs sm:text-sm leading-relaxed text-tas-dark/85">
                          ...
                          <div className="bg-tas-bege/30 p-4 rounded-3xl border border-tas-bege/80 space-y-3">
                            ...
                            <ul className="space-y-2">
                              {selectedProduct.benefits.map((b, idx) => (
                                <li
                                  key={idx}
                                  className="text-xs sm:text-sm flex gap-2 text-tas-dark/95"
                                >
                          ...
                          <div className="space-y-2 border border-tas-bege/60 rounded-3xl p-4 bg-white shadow-xs">
                            <h3 className="font-serif text-xs font-bold text-tas-dark uppercase tracking-wide pb-1.5 border-b border-tas-bege">
                            ...
                            <div className="grid grid-cols-2 gap-y-2 text-xs text-tas-dark divide-y divide-tas-bege/50">
                              <div className="py-1 flex justify-between pr-4">
                                <span className="text-tas-terroir">
                              ...
                              <div className="py-1 flex justify-between pr-4">
                                ...
                                <span className="font-semibold">
                                  {selectedProduct.nutritionalInfo.fat}
                                </span>
                              </div>
                              <div className="py-1 flex justify-between pr-4 border-t-0">
                                <span className="text-tas-terroir">
                                  Fibras Alimentares:
                                </span>{" "}
                                <span className="font-semibold">
                                  {selectedProduct.nutritionalInfo.fiber}
                                </span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2.5">
                              {selectedProduct.reviews.map((r) => (
                                <div
                                  key={r.id}
                                  className="p-3 bg-white rounded-2xl border border-tas-bege/55"
                                >
                                  ...
                                  <div className="flex gap-0.5 text-tas-gold mb-1.5">
                                    {[...Array(5)].map((_, i) => (
                                      <Star
                                        key={i}
                                        className="h-3 w-3 fill-tas-gold text-tas-gold"
                                      />
                                    ))}
                                  </div>
                                  <p className="text-xs text-tas-dark/85 italic leading-tight">
                                    "{r.comment}"
                                  </p>
                                </div>
                              ))}
                      ...
                      >
                          ...
                          <>
                            {cart.length === 0 ? (
                              <div className="text-center py-16 space-y-4">
                                ...
                                <p className="text-sm font-bold text-tas-dark/80">
                                  Seu carrinho de saúde está vazio.
                                </p>
                                <p className="text-xs text-tas-terroir px-6">
                                  Adicione os melhores grãos, chás e castanhas a
                                  granel para começar a cuidar do seu organismo.
                                </p>
                                <button
                                  onClick={() =>
                                    setCurrentView(ActiveView.CATEGORIES)
                              ...
                              <div className="space-y-4">
                                  ...
                                  {cart.map((item) => (
                                    ...
                                    >
                                      ...
                                      <div className="flex-1 text-left">
                                        <h4 className="text-xs font-bold text-tas-dark leading-snug line-clamp-1">
                                          {item.product.name}
                                        </h4>
                                        <span className="text-[10px] text-tas-terroir font-mono">
                                          R${" "}
                                          {item.product.pricePer100g.toFixed(2)}
                                          /100g
                                        </span>
                                ...
                                <div className="bg-tas-cream border border-tas-bege/80 rounded-[2rem] p-4.5 space-y-3 shadow-xs">
                                  <h3 className="font-serif text-xs font-bold text-tas-dark uppercase tracking-wide border-b border-tas-bege pb-2">
                                    Resumo Financeiro da Saúde
                                  </h3>
                        ...
                        {checkoutStep === "shipping" && (
                          <div className="space-y-4 animate-slideup">
                            <h3 className="font-serif text-lg text-tas-dark font-bold mb-1">
                              Escolha o Endereço de Entrega
                            </h3>
                            <div className="p-4 bg-white rounded-3xl border border-tas-bege space-y-4 shadow-xs">
                              <div>
                                <label className="text-[10px] font-bold text-tas-terroir uppercase block mb-1">
                                  Seu Endereço de Cadastro
                                </label>
                                <textarea
                              ...
                              <div className="flex items-center gap-2 text-xs text-tas-terroir">
                                ...
                                <span>
                                  Entrega expressa realizada de 2 a 5 dias úteis
                                  no Brasil.
                                </span>
                              </div>
                            </div>
                        ...
                        {checkoutStep === "payment" && (
                          <div className="space-y-5 animate-slideup">
                            ...
                            <div className="space-y-2 select-none">
                              {/* PIX Option */}
                              <label
                                className={`block p-4 rounded-3xl border cursor-pointer transition-all ${orderPaymentMethod === "PIX" ? "border-tas-gold bg-tas-bege text-tas-dark font-semibold" : "border-gray-150 bg-white hover:border-tas-bege"}`}
                              >
                                <div className="flex items-start justify-between">
                                  <div className="flex gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-[10px] text-tas-terroir font-normal leading-tight">
                                        Ganhe aprovação instantânea automatizada
                                        com QR code.
                                      </p>
                                    </div>
                                  </div>
                              ...
                              <label
                                className={`block p-4 rounded-3xl border cursor-pointer transition-all ${orderPaymentMethod === "WhatsApp Direct" ? "border-tas-gold bg-tas-bege text-tas-dark font-semibold" : "border-gray-150 bg-white hover:border-tas-bege"}`}
                              >
                                <div className="flex items-start justify-between">
                                  <div className="flex gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-[10px] text-tas-terroir font-normal leading-tight">
                                        Geramos uma mensagem dinâmica para
                                        finalizar no balcão!
                                      </p>
                                    </div>
                                  </div>
                                  <input
                                    ...
                                    onChange={() =>
                                      setOrderPaymentMethod("WhatsApp Direct")
                                    }
                                    className="mt-1"
                                  />
                                </div>
                              ...
                              {/* Credit Card Option */}
                              <label
                                className={`block p-4 rounded-3xl border cursor-pointer transition-all ${orderPaymentMethod === "Cartão de Crédito" ? "border-tas-gold bg-tas-bege text-tas-dark font-semibold" : "border-gray-150 bg-white hover:border-tas-bege"}`}
                              >
                                <div className="flex items-start justify-between">
                                  <div className="flex gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-[10px] text-tas-terroir font-normal leading-tight">
                                        Parcele em até 6x sem juros de grãos
                                        saudáveis.
                                      </p>
                                    </div>
                                  </div>
                                  <input
                                    ...
                                    onChange={() =>
                                      setOrderPaymentMethod("Cartão de Crédito")
                                    }
                                    className="mt-1"
                                  />
                                </div>
                            ...
                            {orderPaymentMethod === "Cartão de Crédito" && (
                              <div className="p-4 bg-white rounded-3xl border border-tas-bege space-y-3 shadow-xs animate-slideup">
                                <div>
                                  <label className="text-[9px] uppercase font-bold tracking-wider text-tas-terroir">
                            ...
                            {orderPaymentMethod === "PIX" && (
                              <div className="p-4 bg-tas-cream border border-tas-gold/25 rounded-3xl space-y-4 shadow-xs text-center animate-slideup">
                                <p className="text-xs text-tas-dark/95">
                                  Escaneie o QR Code para pagar ou clique em
                                ...
                                <button
                                  onClick={() => {
                                    navigator.clipboard.writeText(
                                      "00020101021226830014br.gov.bcb.pix2561api.tasgraos.com/pix/pagamento/17794766",
                                    );
                                    setIsCopiedPix(true);
                                    setTimeout(
                                      () => setIsCopiedPix(false),
                                      2000,
                                    );
                                  }}
                            ...
                            <div className="bg-tas-bege/40 p-4 rounded-3xl border border-tas-bege text-xs space-y-1.5 text-tas-dark">
                              <div className="flex justify-between font-bold">
                                <span>Total Geral da Encomenda:</span>{" "}
                                <span className="font-mono text-tas-olive">
                                  R$ {cartTotal.toFixed(2)}
                                </span>
                              </div>
                              <p className="text-[10px] text-tas-terroir text-left">
                                Ao clicar abaixo, você concorda em gerar a
                                solicitação final de compra.
                              </p>
                            </div>
                        ...
                        {checkoutStep === "done" && (
                          <div className="text-center py-12 space-y-6 animate-slideup">
                            ...
                            <div className="space-y-2">
                              <h3 className="font-serif text-xl text-tas-dark font-black">
                                Encomenda Realizada! 🌱✨
                              </h3>
                              <p className="text-xs text-tas-terroir font-semibold">
                                Parabéns pela sua escolha por saúde e nutrição
                                superior.
                              </p>
                            </div>

                            <div className="p-4 bg-white border border-tas-bege rounded-3xl text-left space-y-3 shadow-xs">
                              <p className="text-xs text-tas-dark">
                                Nossa equipe já está separando seus produtos a
                                granel com luvas de proteção em embalagens
                                vedadas.
                              </p>
                              <div className="text-[11px] font-mono bg-tas-bege text-tas-dark/95 p-2 rounded-xl border border-tas-gold/15">
                                • Código Rastreamento Correios:{" "}
                                <span className="font-bold text-tas-olive">
                                  TG
                                  {Math.floor(
                              ...
                              <p className="text-[10px] text-gray-400">
                                Suas notas fiscais e atualizações de entrega
                                foram disponibilizados na aba Perfil.
                              </p>
                            </div>
                    ...
                    {currentView === ActiveView.FAVORITES && (
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="space-y-4 pb-20 text-left"
                      >
                        {favorites.length === 0 ? (
                          <div className="text-center py-16 space-y-3">
                            ...
                            <p className="text-xs text-tas-terroir px-8 leading-relaxed">
                              Adicione um coração na tela dos produtos e veja-os
                              reunidos aqui para agilizar sua recompra natural.
                            </p>
                          </div>
                        ) : (
                          <div className="grid grid-cols-2 gap-3.5">
                              ...
                              .map((p) => (
                                ...
                                >
                                  ...
                                  <div
                                    ...
                                    className="cursor-pointer"
                                  >
                                    <h3 className="text-xs font-bold text-tas-dark line-clamp-1">
                                      {p.name}
                                    </h3>
                                    <p className="text-[10px] text-tas-terroir">
                                      {p.category}
                                    ...
                                    <div className="flex justify-between items-center mt-1">
                                      ...
                                      <button
                                        onClick={(e) => {
                                          ...
                                          handleAddProductToCart(p, 250);
                                          alert(
                                            `${p.name} (250g) inserido no carrinho! 🛒`,
                                          );
                                        }}
                        ...
                        {!selectedArticle ? (
                          <div className="space-y-5">
                            <p className="text-xs sm:text-sm text-tas-dark/85 leading-relaxed bg-white border border-tas-bege p-3.5 rounded-2xl">
                              Consulte as publicações oficiais das nossas redes
                              sociais. Entenda do ponto de vista terapêutico
                              como funcionam nossos superalimentos.
                            </p>
                                    ...
                                    <div className="flex justify-between items-center mt-3 pt-3 border-t border-tas-bege/60">
                                      <div className="flex gap-1">
                                        {art.tags.slice(0, 2).map((t) => (
                                          <span
                                            key={t}
                                            className="text-[9px] text-tas-terroir font-semibold"
                                          >
                                            #{t}
                                          </span>
                                        ))}
                          ...
                          >
                            ...
                            >
                              <ArrowLeft className="h-4 w-4" /> Voltar aos
                              Artigos
                            </button>
                            ...
                            <p className="text-xs text-tas-terroir font-medium italic">
                            ...
                            <div className="bg-tas-bege/35 border border-tas-bege p-4 rounded-3xl space-y-2.5 mt-4">
                              ...
                              <ul className="space-y-2 pl-1.5">
                                {selectedArticle.tips.map((t, idx) => (
                                  ...
                                  >
                                    <span className="text-tas-olive font-bold">
                                      •
                                    </span>
                                    <span>{t}</span>
                                  </li>
                      ...
                      >
                        ...
                        <div className="bg-white p-5 rounded-3xl border border-tas-bege/70 flex gap-4 items-center shadow-3xs">
                          ...
                          <div className="text-left overflow-hidden">
                            <h3 className="text-sm sm:text-base font-bold text-tas-dark leading-snug truncate">
                              {user.name || "Visitante"}
                            </h3>
                            <p className="text-[11px] text-tas-terroir truncate">
                              {user.email || "Acesso como Visitante"}
                            </p>
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                          ...
                          >
                            <User
                              className={`h-5 w-5 mb-1.5 ${activeProfileTab === "perfil" ? "text-white" : "text-tas-gold"}`}
                            />
                          ...
                          >
                            <ShoppingBag
                              className={`h-5 w-5 mb-1.5 ${activeProfileTab === "history" ? "text-white" : "text-tas-gold"}`}
                            />
                            <span className="text-[11px] font-bold tracking-tight">
                              Histórico
                            </span>
                          ...
                          >
                            <Settings
                              className={`h-5 w-5 mb-1.5 ${activeProfileTab === "settings" ? "text-white" : "text-tas-gold"}`}
                            />
                            <span className="text-[11px] font-bold tracking-tight">
                              Configurações
                            </span>
                          ...
                          >
                            <LogOut
                              className={`h-5 w-5 mb-1.5 ${activeProfileTab === "logout" ? "text-red-600" : "text-red-500"}`}
                            />
                            <span className="text-[11px] font-bold tracking-tight">
                              Sair da Conta
                            </span>
                          ...
                          {activeProfileTab === "perfil" && (
                            ...
                            >
                              ...
                              <div className="bg-white p-5 rounded-3xl border border-tas-bege/70 space-y-3 shadow-3xs">
                                <h4 className="font-serif text-xs font-bold text-tas-dark uppercase tracking-wide border-b border-tas-bege/40 pb-2">
                                  Informações de Registro
                                </h4>

                                <div className="grid grid-cols-3 text-xs py-1">
                                  <span className="text-tas-terroir font-medium">
                                    Nome:
                                  </span>
                                  <span className="col-span-2 text-tas-dark font-semibold break-all">
                                ...
                                <div className="grid grid-cols-3 text-xs py-1 border-t border-tas-bege/20">
                                  ...
                                  <span className="col-span-2 text-tas-dark font-semibold">
                                    {user.phone ||
                                      "Nenhum telefone configurado"}
                                  </span>
                                </div>
                                ...
                                <div className="grid grid-cols-3 text-xs py-1 border-t border-tas-bege/20">
                                  ...
                                  <span className="col-span-2 text-tas-dark font-semibold leading-relaxed">
                                    {user.address ||
                                      "Nenhum endereço de entrega configurado"}
                                  </span>
                                </div>
                              </div>
                            </motion.div>
                          ...
                          {activeProfileTab === "history" && (
                            ...
                            >
                              <div className="flex justify-between items-center px-1">
                                <h3 className="font-serif text-xs font-bold text-tas-dark uppercase tracking-wide">
                                  Histórico de Pedidos
                                </h3>
                                <span className="text-[10px] text-gray-400">
                                  Total de {user.orderHistory.length}
                                </span>
                              </div>
                              ...
                              {user.orderHistory.length === 0 ? (
                                <div className="bg-white p-6 rounded-3xl border border-tas-bege/60 text-center space-y-2">
                                  <p className="text-xs text-tas-terroir">
                                    Nenhum histórico de pedidos encontrado.
                                  </p>
                                  <p className="text-[10px] text-gray-400 font-medium">
                                    Suas compras aparecerão aqui após finalizar
                                    o carrinho.
                                  </p>
                                </div>
                              ) : (
                                  ...
                                  {user.orderHistory.map((order) => (
                                    ...
                                    >
                                      <div className="flex justify-between items-center pb-2 border-b border-tas-bege/60 mb-2">
                                        <span className="font-bold text-tas-dark">
                                          {order.id}
                                        </span>
                                      ...
                                      <div className="space-y-1 mb-3">
                                        {order.items.map((item, idx) => (
                                          <div
                                            key={idx}
                                            className="flex justify-between text-tas-dark/90 text-[11px]"
                                          >
                                            <span>
                                              • {item.product.name} (
                                              {item.weightGrams}g)
                                      ...
                                      <div className="bg-tas-bege/25 p-2.5 rounded-2xl border border-tas-bege mb-2">
                                        <div className="flex justify-between items-center mb-1 text-[10px]">
                                          <span className="text-tas-terroir">
                                            Rastreio:{" "}
                                            <span className="font-mono text-tas-dark font-bold">
                                              {order.trackerCode}
                                            </span>
                                          </span>
                                      ...
                                      <div className="flex justify-between items-center text-[11px] pt-1">
                                        ...
                                        <span className="font-mono font-black text-tas-olive">
                          ...
                          {activeProfileTab === "settings" && (
                            ...
                            >
                              <div className="bg-white p-5 rounded-3xl border border-tas-bege/70 space-y-4 shadow-3xs">
                                ...
                                <div className="space-y-3.5">
                                  <div>
                                    <label className="text-[10px] uppercase font-bold text-tas-terroir block mb-1">
                                      Nome Completo
                                    </label>
                                    <input
                                      type="text"
                                ...
                                <div className="bg-tas-bege/20 border border-tas-bege/80 p-4.5 rounded-3xl space-y-3">
                                  <div className="flex items-center gap-2.5">
                                    ...
                                    <div className="text-left">
                                      <h4 className="font-serif text-xs font-bold text-tas-dark uppercase tracking-wide">
                                        Banco de Dados
                                      </h4>
                                      <p className="text-[10px] text-tas-terroir text-left">
                                        Gerenciar configurações avançadas do
                                        JSON
                                      </p>
                                    </div>
                                  </div>
                            ...
                            >
                              <div className="bg-white border border-red-100 p-6 rounded-3xl text-center space-y-4 shadow-3xs">
                                ...
                                <div className="space-y-1">
                                  <h4 className="font-serif font-black text-tas-dark text-base">
                                    Terminar Sessão
                                  </h4>
                                  <p className="text-xs text-tas-terroir">
                      ...
                      >
                        ...
                        {user.email !== "lasolucoesdigitais007@gmail.com" ? (
                          <div className="bg-white p-8 rounded-3xl border border-tas-bege text-center space-y-5 max-w-sm mx-auto shadow-sm my-12">
                            ...
                            <div>
                              <h3 className="font-serif text-lg font-bold text-tas-dark">
                                Acesso Restrito
                              </h3>
                              <p className="text-xs text-tas-terroir mt-1 leading-relaxed">
                                Oops! Esta área é exclusiva para o gestor
                                oficial da **Tas Grãos**. Seu e-mail (
                                {user.email || "Convidado"}) não tem permissão
                                de escrita.
                              </p>
                            </div>
                            <button
                          ...
                          <>
                            ...
                            <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-tas-bege/60 shadow-xs">
                              <button
                                onClick={() =>
                                  setCurrentView(ActiveView.PROFILE)
                                }
                              ...
                              <div className="text-right">
                                <span className="text-[9px] uppercase font-bold tracking-widest text-tas-gold">
                                  Painel do Gestor
                                </span>
                                <h2 className="font-serif text-sm font-black text-tas-dark">
                                  Tas Grãos Editor
                                </h2>
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 xs:grid-cols-4 gap-2.5">
                              <div className="bg-white p-3 rounded-2xl border border-tas-bege/60 flex flex-col justify-between shadow-3xs">
                                <span className="text-[9px] text-tas-terroir uppercase font-bold">
                                  Total Produtos
                                </span>
                                <p className="text-lg font-mono font-black text-tas-dark mt-1">
                                  {products.length}
                                </p>
                              </div>
                              <div className="bg-white p-3 rounded-2xl border border-tas-bege/60 flex flex-col justify-between shadow-3xs">
                              ...
                              <div className="bg-white p-3 rounded-2xl border border-tas-bege/60 flex flex-col justify-between shadow-3xs">
                                ...
                                <p className="text-lg font-mono font-black text-tas-gold mt-1">
                                  {promotions.length}
                                </p>
                              </div>
                              <div className="bg-white p-3 rounded-2xl border border-tas-bege/60 flex flex-col justify-between shadow-3xs">
                                <span className="text-[9px] text-tas-terroir uppercase font-bold text-tas-olive">
                                  Banners
                                </span>
                                <p className="text-lg font-mono font-black mt-1 text-tas-olive">
                                  {banners.length}
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex gap-1.5 overflow-x-auto pb-1 shrink-0 app-scrollbar select-none">
                              ...
                              >
                                <Database className="h-3.5 w-3.5 text-tas-gold" />{" "}
                                Avançado (JSON)
                              </button>
                            </div>
                                ...
                                {!isAddingItem && !editingProd ? (
                                  <>
                                    ...
                                    {products.length === 0 ? (
                                      <div className="text-center py-16 bg-white rounded-2xl border border-tas-bege/60">
                                        <p className="text-xs text-tas-terroir font-bold">
                                          Nenhum produto cadastrado no
                                          Firestore.
                                        </p>
                                        <p className="text-[10px] text-gray-400 mt-0.5">
                                          Clique em "Semear" acima ou cadastre
                                          um.
                                        </p>
                                      </div>
                                    ) : (
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {products.map((p) => (
                                          <div
                                            key={p.id}
                                            className="bg-white p-3.5 rounded-2xl border border-tas-bege/60 flex justify-between items-center text-xs shadow-3xs hover:border-tas-gold/60 transition-colors"
                                          >
                                            <div className="flex gap-2.5 items-center min-w-0">
                                              <img
                                                src={p.imageUrl}
                                                alt={p.name}
                                                className="h-12 w-12 object-cover rounded-xl bg-tas-bege shrink-0 border border-tas-bege/40"
                                              />
                                              <div className="min-w-0">
                                                <p className="font-bold text-tas-dark truncate">
                                                  {p.name}
                                                </p>
                                                <div className="flex items-center gap-1.5 mt-0.5">
                                                  <span className="text-[9px] bg-tas-bege text-tas-olive font-bold px-1.5 py-0.2 rounded-md">
                                                    {p.category}
                                                  </span>
                                                  <span className="text-[10px] text-tas-dark font-mono font-bold">
                                                    R${" "}
                                                    {p.pricePer100g.toFixed(2)}
                                                    /100g
                                                  </span>
                                                </div>
                                                <p className="text-[9px] text-gray-400 mt-1 flex items-center gap-1">
                                                  <span>
                                                    Estoque:{" "}
                                                    <strong className="text-tas-terroir">
                                                      {p.stock !== undefined
                                                        ? p.stock
                                                        : 100}
                                                      {p.stockUnit || "g"}
                                                  ...
                                                  {p.promoPrice && (
                                                    <span className="text-tas-gold font-bold">
                                                      🏷️ Promo: R${" "}
                                                  ...
                                                  setConfirmDialog({
                                                    ...
                                                    onConfirm: async () => {
                                                      try {
                                                        ...
                                                        alert(
                                                          "Produto removido de imediato do Firestore! 🗑️✅",
                                                        );
                                                      } catch (err) {
                                                        handleFirestoreError(
                                                          err,
                                  ...
                                  <div className="bg-white p-5 rounded-3xl border border-tas-bege space-y-4 text-xs">
                                    <div className="border-b pb-2.5 flex justify-between items-center">
                                      <h3 className="font-serif font-black text-tas-dark text-sm">
                                        Novo Produto
                                      </h3>
                                      <span className="text-[9px] bg-tas-gold text-white font-bold px-2 py-0.5 rounded-full uppercase">
                                        Criação
                                      </span>
                                    </div>

                                    <div className="space-y-1 text-left">
                                      <label className="text-[10px] uppercase font-bold text-tas-terroir">
                                        Nome do Produto
                                      </label>
                                      <input
                                        type="text"
                                    ...
                                    <div className="grid grid-cols-2 gap-2.5 text-left">
                                      <div className="space-y-1">
                                        <label className="text-[10px] uppercase font-bold text-tas-terroir block">
                                          Categoria
                                        </label>
                                        {categories.length > 0 ? (
                                    ...
                                    <div className="space-y-1 text-left">
                                      <label className="text-[10px] uppercase font-bold text-tas-terroir">
                                        Benefícios do Produto (separados por
                                        vírgula)
                                      </label>
                                      <input
                                        type="text"
                                        ...
                                        className="w-full p-2.5 border border-tas-bege rounded-xl outline-none text-xs"
                                        value={
                                          Array.isArray(newProd.benefits)
                                            ? newProd.benefits.join(", ")
                                            : ""
                                        }
                                        onChange={(e) =>
                                          setNewProd({
                                    ...
                                    <div className="space-y-1 text-left border-t pt-3.5">
                                      <p className="text-[10px] uppercase font-bold text-tas-terroir mb-1">
                                        Informações Nutricionais (p/ 100g)
                                      </p>
                                    ...
                                    <div className="space-y-1 text-left">
                                      ...
                                      <div className="flex flex-col gap-2">
                                        <input
                                          type="file"
                                          accept="image/*"
                                          className="w-full text-xs text-tas-dark file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-gray-100 file:text-tas-dark hover:file:bg-gray-200 cursor-pointer"
                                          onChange={(e) => {
                                            const file = e.target.files?.[0];
                                            if (file) {
                                              if (file.size > 900 * 1024) {
                                                alert("A imagem é muito grande. Escolha uma imagem de até 900KB.");
                                                e.target.value = '';
                                                return;
                                              }
                                        ...
                                        <input
                                          type="text"
                                          placeholder="Ou insira o link da imagem..."
                                          className="w-full p-2.5 border border-tas-bege rounded-xl outline-none text-xs"
                                      ...
                                      {newProd.imageUrl && (
                                        <div className="mt-2 h-16 w-16 relative rounded-xl overflow-hidden border border-tas-bege/40 bg-tas-bege shrink-0">
                                          <img src={newProd.imageUrl} alt="Preview" className="h-full w-full object-cover" />
                                        </div>
                                      )}
                                    </div>
                                        ...
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            const payload = {
                                              ...
                                              description:
                                                newProd.description ||
                                                "Alimento puro de alto valor biológico.",
                                              benefits:
                                                newProd.benefits &&
                                                newProd.benefits.length > 0
                                                  ? newProd.benefits
                                                  : [
                                                      "Puro e livre de aromatizantes",
                                                      "Excelente procedência natural",
                                                    ],
                                              category: cat,
                                              pricePer100g: Number(
                                                newProd.pricePer100g,
                                              ),
                                              promoPrice: (newProd as any)
                                                .promoPrice
                                                ? Number(
                                                    (newProd as any).promoPrice,
                                                  )
                                            ...
                                            await setDoc(
                                              doc(db, "products", createdId),
                                              payload,
                                            );
                                            setIsAddingItem(false);
                                            alert(
                                              "Excelente! Produto criado e salvo no Firestore! 🚀🎉",
                                            );
                                          } catch (error) {
                                            handleFirestoreError(
                                              error,
                                  ...
                                  <div className="bg-white p-5 rounded-3xl border border-tas-bege space-y-4 text-xs">
                                    <div className="border-b pb-2.5 flex justify-between items-center">
                                      <h3 className="font-serif font-black text-tas-dark text-sm">
                                        Editar Produto (Firestore)
                                      </h3>
                                      <span className="text-[9px] bg-tas-olive text-white font-bold px-2 py-0.5 rounded-full uppercase">
                                        Edição
                                      </span>
                                    </div>
                                    ...
                                    <div className="space-y-1 text-left">
                                      ...
                                      <input
                                        ...
                                        className="w-full p-2.5 border border-tas-bege rounded-xl outline-none text-xs"
                                        value={
                                          Array.isArray(editingProd.benefits)
                                            ? editingProd.benefits.join(", ")
                                            : ""
                                        }
                                        onChange={(e) =>
                                          setEditingProd({
                                    ...
                                    <div className="space-y-1 text-left">
                                        ...
                                        <input
                                          ...
                                          onChange={(e) => {
                                            ...
                                            if (file) {
                                              ...
                                              const reader = new FileReader();
                                              reader.onloadend = () => {
                                                setEditingProd({ ...editingProd, imageUrl: reader.result as string });
                                              };
                                              reader.readAsDataURL(file);
                                            }
                                          }}
                                      ...
                                      {editingProd.imageUrl && (
                                        <div className="mt-2 h-16 w-16 relative rounded-xl overflow-hidden border border-tas-bege/40 bg-tas-bege shrink-0">
                                          <img src={editingProd.imageUrl} alt="Preview" className="h-full w-full object-cover" />
                                        </div>
                                      )}
                                    </div>
                                    ...
                                    <div className="flex gap-2 pt-3 border-t">
                                      <button
                                        onClick={async () => {
                                          if (
                                            !editingProd.name ||
                                            !editingProd.pricePer100g ||
                                            !editingProd.category
                                          ) {
                                            alert(
                                              "Preencha Nome, Preço e Categoria!",
                                            );
                                          ...
                                          try {
                                            ...
                                            const payload = {
                                              ...
                                              promoPrice: editingProd.promoPrice
                                                ? Number(editingProd.promoPrice)
                                                : null,
                                              stock: Number(
                                                editingProd.stock !== undefined
                                                  ? editingProd.stock
                                            ...
                                            setEditingProd(null);
                                            alert(
                                              "Excelente! Alterações do produto salvas com sucesso no Firestore! 💾✅",
                                            );
                                          } catch (error) {
                                            handleFirestoreError(
                                              error,
                                              OperationType.WRITE,
                                              `update-${editingProd.id}`,
                                            );
                                          }
                                        }}
                                        className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-center cursor-pointer transition-colors"
                            ...
                            {showAdminTab === "categories" && (
                              <div className="space-y-4">
                                  ...
                                  <button
                                    onClick={async () => {
                                      ...
                                      if (categories.includes(formattedCat)) {
                                        alert(
                                          "Essa categoria já existe no seu catálogo!",
                                        );
                                        return;
                                      }
                                      try {
                                        await setDoc(
                                          doc(db, "categories", formattedCat),
                                          {
                                            id: formattedCat,
                                            name: formattedCat,
                                            isActive: true,
                                        ...
                                        setNewCatName("");
                                        alert(
                                          "Categoria salva no Firestore com sucesso! 🌱✅",
                                        );
                                      } catch (error) {
                                        handleFirestoreError(
                                          error,
                                ...
                                <div className="bg-white p-4 rounded-3xl border border-tas-bege space-y-3 shadow-3xs">
                                  <h3 className="text-xs uppercase font-bold text-tas-terroir border-b pb-1.5 text-left">
                                        ...
                                        >
                                          ...
                                          <button
                                            onClick={async () => {
                                              if (
                                                confirm(
                                                  `Remover a categoria "${c}" definitivamente do Firestore? Isso não afetará os produtos associados.`,
                                                )
                                              ) {
                                                try {
                                                  await deleteDoc(
                                                  ...
                                                  alert(
                                                    "Categoria deletada do Firestore! 🗑️✅",
                                                  );
                                                } catch (error) {
                                                  handleFirestoreError(
                                                    error,
                                ...
                                {!isAddingItem && !editingPromo ? (
                                  <>
                                    <button
                                      ...
                                      className="w-full py-3 bg-tas-olive hover:bg-tas-olive-dark text-white rounded-xl text-xs font-bold font-serif flex items-center justify-center gap-1.5 cursor-pointer shadow-3xs transition-transform active:scale-98"
                                    >
                                      <Plus className="h-4 w-4" /> Publicar Nova
                                      Promoção (Firestore)
                                    </button>
                                    ...
                                    {promotions.length === 0 ? (
                                      <div className="text-center py-16 bg-white rounded-2xl border border-tas-bege/60">
                                        ...
                                        <p className="text-[10px] text-gray-400 mt-0.5">
                                          Promoções ativas com cupons e
                                          descontos aparecerão aqui.
                                        </p>
                                      </div>
                                    ) : (
                                      <div className="space-y-3">
                                        {promotions.map((promo) => (
                                          <div
                                            key={promo.id}
                                            className="bg-white p-4 rounded-2xl border border-tas-bege/60 flex justify-between items-center text-xs shadow-3xs"
                                          >
                                            <div className="flex gap-3 items-center min-w-0 font-sans">
                                              ...
                                              <div className="min-w-0 text-left">
                                                ...
                                                <p className="text-[10px] text-tas-olive font-semibold mt-0.5">
                                                  Expira em:{" "}
                                                  {new Date(
                                                    promo.expiryDate,
                                                  ).toLocaleDateString()}
                                            ...
                                            <div className="flex items-center gap-1.5">
                                              ...
                                              <button
                                                onClick={async () => {
                                                  if (
                                                    !confirm(
                                                      `Excluir a campanha "${promo.title}" do Firestore?`,
                                                    )
                                                  )
                                                    return;
                                                  try {
                                                    ...
                                                    alert(
                                                      "Promoção deletada do Firestore! 🗑️✅",
                                                    );
                                                  } catch (err) {
                                                    handleFirestoreError(
                                                      err,
                                  ...
                                  <div className="bg-white p-5 rounded-3xl border border-tas-bege space-y-4 text-xs font-sans">
                                    <div className="border-b pb-2.5 flex justify-between items-center">
                                      ...
                                      <span className="text-[10px] bg-tas-gold text-white font-bold px-2.5 py-0.5 rounded-full uppercase">
                                        Nova
                                      </span>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2.5 text-left font-sans">
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        <input
                                          ...
                                          className="w-full p-2.5 border border-tas-bege rounded-xl outline-none"
                                          value={
                                            newPromo.expiryDate || "2026-12-31"
                                          }
                                          onChange={(e) =>
                                            setNewPromo({
                                              ...newPromo,
                                    ...
                                    <div className="space-y-2 text-left">
                                      ...
                                      <div className="max-h-40 overflow-y-auto border border-tas-bege/60 rounded-xl p-2.5 space-y-1.5">
                                        {products.map((prod) => (
                                          <label
                                            key={prod.id}
                                            className="flex items-center gap-2 py-0.5 cursor-pointer hover:bg-tas-bege/20"
                                          >
                                            <input
                                              type="checkbox"
                                              checked={newPromo.selectedProductIds?.includes(
                                                prod.id,
                                              )}
                                              onChange={(e) => {
                                                const checked =
                                      ...
                                      <button
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            await setDoc(
                                              doc(db, "promotions", promoId),
                                              payload,
                                            );
                                            setIsAddingItem(false);
                                            alert(
                                              "Sucesso! Campanha de Desconto publicada no Firestore! 🏷️🎉",
                                            );
                                          } catch (err) {
                                            handleFirestoreError(
                                              err,
                                  ...
                                  <div className="bg-white p-5 rounded-3xl border border-tas-bege space-y-4 text-xs font-sans">
                                    <div className="border-b pb-2.5 flex justify-between items-center font-sans">
                                      <h3 className="font-serif font-black text-tas-dark text-sm font-serif">
                                        Editar Campanha Promocional
                                      </h3>
                                      <span className="text-[10px] bg-tas-olive text-white font-bold px-2.5 py-0.5 rounded-full uppercase">
                                        Editar
                                      </span>
                                    </div>

                                    <div className="space-y-1 text-left">
                                      <label className="text-[10px] uppercase font-bold text-tas-terroir font-sans">
                                        Título de Chamada
                                      </label>
                                      <input
                                        type="text"
                                    ...
                                    <div className="space-y-1 text-left font-sans">
                                      <label className="text-[10px] uppercase font-bold text-tas-terroir font-sans font-bold">
                                        URL da Imagem
                                      </label>
                                      <input
                                        type="text"
                                      ...
                                      <div className="max-h-40 overflow-y-auto border border-tas-bege/60 rounded-xl p-2.5 space-y-1.5">
                                        {products.map((prod) => (
                                          ...
                                          >
                                            <input
                                              type="checkbox"
                                              checked={editingPromo.selectedProductIds?.includes(
                                                prod.id,
                                              )}
                                              onChange={(e) => {
                                                const checked =
                                                  e.target.checked;
                                                const currentIds =
                                                  editingPromo.selectedProductIds ||
                                                  [];
                                                if (checked) {
                                                  setEditingPromo({
                                                    ...editingPromo,
                                      ...
                                      <button
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            alert(
                                              "Edições da promoção aplicadas no Firestore! 💾🏷️",
                                            );
                                          } catch (err) {
                                            handleFirestoreError(
                                              err,
                                              OperationType.WRITE,
                                              `update-promo-${editingPromo.id}`,
                                            );
                                          }
                                        }}
                                        className="flex-grow py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-center cursor-pointer transition-colors"
                              ...
                              <div className="space-y-4 font-sans">
                                {!isAddingItem && !editingBanner ? (
                                  <>
                                    ...
                                    >
                                      <Plus className="h-4 w-4" /> Cadastrar
                                      Novo Banner Slider (Firestore)
                                    </button>

                                    {banners.length === 0 ? (
                                      <div className="text-center py-16 bg-white rounded-2xl border border-tas-bege/60">
                                        <p className="text-xs text-tas-terroir font-bold">
                                          Nenhum banner slider registrado no
                                          momento.
                                        </p>
                                        <p className="text-[10px] text-gray-400 mt-0.5">
                                          Gerencie os slides ambientais da
                                          página de início aqui.
                                        </p>
                                      </div>
                                    ) : (
                                      <div className="space-y-3">
                                          ...
                                          >
                                            <div className="flex gap-3 items-center min-w-0">
                                              <img
                                                src={slide.image}
                                                alt={slide.title}
                                                className="h-12 w-20 object-cover rounded-xl bg-tas-bege shrink-0 border border-tas-bege/40"
                                              />
                                              <div className="min-w-0 text-left">
                                                <span className="text-[8px] uppercase font-bold tracking-wider text-tas-gold">
                                                  {slide.tag}
                                                </span>
                                                <p className="font-bold text-tas-dark truncate mt-0.5">
                                                  {slide.title}
                                                </p>
                                                <p className="text-[9px] text-tas-terroir truncate mt-0.5">
                                                  {slide.desc}
                                                </p>
                                              </div>
                                            </div>
                                            <div className="flex items-center gap-1.5 font-sans">
                                              ...
                                              <button
                                                onClick={async () => {
                                                  if (
                                                    !confirm(
                                                      `Excluir o slide de banner "${slide.title}" do Firestore?`,
                                                    )
                                                  )
                                                    return;
                                                  try {
                                                    ...
                                                    alert(
                                                      "Banner slider de-ativado com sucesso! 🗑️✅",
                                                    );
                                                  } catch (err) {
                                                    handleFirestoreError(
                                                      err,
                                      ...
                                      <button
                                        onClick={async () => {
                                          if (
                                            ...
                                            alert(
                                              "Insira título e imagem para o banner!",
                                            );
                                            return;
                                          }
                                          try {
                                            ...
                                            const payload = {
                                              ...
                                              desc:
                                                newBanner.desc ||
                                                "Qualidade excepcional direto do produtor ancestral.",
                                              tag:
                                                newBanner.tag || "Lançamento",
                                            ...
                                            setIsAddingItem(false);
                                            alert(
                                              "Sucesso! Slide de Banner cadastrado no Firestore! 🌌🥳",
                                            );
                                          } catch (err) {
                                            handleFirestoreError(
                                              err,
                                  ...
                                  <div className="bg-white p-5 rounded-3xl border border-tas-bege space-y-4 text-xs font-sans">
                                    <div className="border-b pb-2.5 flex justify-between items-center">
                                      <h3 className="font-serif font-black text-tas-dark text-sm">
                                        Editar Slide de Destaque
                                      </h3>
                                      <span className="text-[10px] bg-tas-olive text-white font-bold px-2.5 py-0.5 rounded-full uppercase font-sans">
                                        Editar Slide
                                      </span>
                                    </div>
                                    ...
                                    <div className="space-y-1 text-left font-sans">
                                      <label className="text-[10px] uppercase font-bold text-tas-terroir font-bold">
                                        Título Principal
                                      </label>
                                      <input
                                        type="text"
                                      ...
                                      <button
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            alert(
                                              "Slide de banner salvo com sucesso no Firestore! 💾🌌",
                                            );
                                          } catch (err) {
                                            handleFirestoreError(
                                              err,
                                              OperationType.WRITE,
                                              `update-banner-${editingBanner.id}`,
                                            );
                                          }
                                        }}
                                        className="flex-grow py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-center cursor-pointer transition-colors"
                              ...
                              <div className="space-y-6 text-left animate-slideup">
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                  <div className="bg-[#FAF8F5] p-4 rounded-3xl border border-tas-bege/60 shadow-3xs">
                                    <span className="text-[10px] text-tas-terroir uppercase font-bold tracking-wider">
                                      Faturamento Geral
                                    </span>
                                    <div className="flex items-baseline gap-1 mt-2">
                                      <span className="text-xs font-bold text-tas-gold">
                                        R$
                                      </span>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <div className="bg-white p-5 rounded-3xl border border-tas-bege/70 shadow-3xs">
                                    <h4 className="font-serif text-xs font-black text-tas-dark uppercase tracking-wider mb-4">
                                  ...
                                  <div className="bg-white p-5 rounded-3xl border border-tas-bege/70 shadow-3xs flex flex-col justify-between">
                                    <div>
                                      <h4 className="font-serif text-xs font-black text-tas-dark uppercase tracking-wider mb-2">
                                        Simulador de Vendas
                                      </h4>
                                      <p className="text-[11px] text-tas-terroir leading-relaxed">
                                        Utilize este atalho inteligente para
                                        criar pedidos fictícios de teste e
                                        validar o faturamento e painéis
                                    ...
                                    <div className="mt-4 flex flex-col xs:flex-row gap-2">
                                      <button
                                        onClick={async () => {
                                          if (
                                            !confirm(
                                              "Deseja semear pedidos simulados de demonstração na base do Firestore para ativar os painéis financeiros?",
                                            )
                                          )
                                            return;
                                          try {
                                            const dummyOrders = [
                                              {
                                                ...
                                                address:
                                                  "Rua Bela Cintra, 450 - Consolação, São Paulo - SP",
                                                userId: "simulated_user_1",
                                                customerName:
                                                  "Larissa Souza Alfenas",
                                                customerEmail:
                                                  "larissa.souza@yahoo.com.br",
                                                customerPhone:
                                                  "(11) 98765-4321",
                                                items: [],
                                              },
                                              {
                                                ...
                                                address:
                                                  "Alameda Lorena, 1200 - Jardins, São Paulo - SP",
                                                userId: "simulated_user_2",
                                                customerName:
                                                  "Bruno de Alencar",
                                                customerEmail:
                                                  "bruno_alencar@gmail.com",
                                                customerPhone:
                                                  "(11) 91234-5678",
                                                items: [],
                                              },
                                              {
                                                ...
                                                address:
                                                  "Av. Oceanica, 820 - Copacabana, Rio de Janeiro - RJ",
                                                userId: "simulated_user_3",
                                                customerName:
                                                  "Ana Maria Vasconcelos",
                                                customerEmail:
                                                  "anamaria.vascon@gmail.com",
                                                customerPhone:
                                                  "(21) 99887-1122",
                                                items: [],
                                              },
                                            ...
                                            alert(
                                              "Parabéns! Pedidos de demonstração injetados com total sucesso no banco central!",
                                            );
                                          } catch (err) {
                                            console.error(err);
                                            alert(
                                              "Erro ao semear pedidos de simulação.",
                                            );
                                          }
                                        }}
                                      ...
                                      {allOrders.length > 0 && (
                                        <button
                                          onClick={async () => {
                                            if (
                                              !confirm(
                                                "Deseja deletar TODOS os pedidos registrados no banco de dados do Firestore?",
                                              )
                                            )
                                              return;
                                            try {
                                              ...
                                              alert(
                                                "Tabela de pedidos limpa com absoluto sucesso!",
                                              );
                                            } catch (err) {
                                              console.error(err);
                                              alert(
                                                "Erro ao limpar base de pedidos.",
                                              );
                                            }
                                          }}
                                ...
                                <div className="bg-white rounded-3xl border border-tas-bege/70 overflow-hidden shadow-3xs">
                                  <div className="p-4 border-b border-tas-bege/60 flex justify-between items-center bg-[#FAF8F5]">
                                    <div>
                                      <h3 className="font-serif text-sm font-black text-tas-dark">
                                        Lista Central de Pedidos e Entregas
                                      </h3>
                                      <p className="text-[10px] text-tas-terroir">
                                        Monitore clientes, controle faturamento,
                                        edite status e coordene despachos.
                                      </p>
                                    </div>
                                  ...
                                  {allOrders.length === 0 ? (
                                    <div className="p-10 text-center text-tas-terroir font-medium text-xs space-y-2">
                                      <p className="text-sm">
                                        📭 Nenhum pedido recebido ou cadastrado
                                        ainda.
                                      </p>
                                      <p className="text-[11px] text-gray-400">
                                        Ative o simulador ao lado para povoar o
                                        faturamento com dados premium
                                        instantaneamente.
                                      </p>
                                    </div>
                                  ) : (
                                            ...
                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                                              <div className="space-y-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                  <span className="font-mono font-black text-tas-dark text-sm">
                                                    {order.id}
                                                  </span>
                                                  <span className="text-[10px] text-gray-400">
                                                    {order.date}
                                              ...
                                              <div className="flex items-center gap-3 justify-between md:justify-end">
                                                <div className="text-right">
                                                  <span className="text-[9px] text-[#D4AF37] uppercase font-bold block leading-none">
                                                    Total
                                                  </span>
                                                ...
                                                <div className="flex items-center gap-2">
                                                  ...
                                                  <select
                                                    ...
                                                    onChange={async (e) => {
                                                      ...
                                                      try {
                                                        ...
                                                        alert(
                                                          "Sem permissão ou falha de rede para salvar o status.",
                                                        );
                                                      }
                                                    }}
                                                  ...
                                                  <button
                                                    onClick={async () => {
                                                      if (
                                                        !confirm(
                                                          `Deseja apagar permanentemente o pedido ${order.id} da base de dados?`,
                                                        )
                                                      )
                                                        return;
                                                      try {
                                                        ...
                                                        alert(
                                                          "Erro de permissão ou rede ao excluir o registro.",
                                                        );
                                                      }
                                                    }}
                                            ...
                                            {isExpanded && (
                                              <div className="bg-gray-50/70 p-4 rounded-2xl border border-tas-bege/50 space-y-3.5 mt-2">
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px]">
                                                  <div className="space-y-1">
                                                    <h5 className="font-extrabold text-[#D4AF37] uppercase text-[9px] tracking-wider">
                                                      Ficha do Cliente
                                                    </h5>
                                                    <p>
                                                      <strong>Nome:</strong>{" "}
                                                  ...
                                                  <div className="space-y-1.5 flex flex-col items-start justify-center">
                                                    <h5 className="font-extrabold text-[#D4AF37] uppercase text-[9px] tracking-wider mb-1">
                                                      Ações Logísticas
                                                    </h5>
                                                ...
                                                <div className="border-t border-tas-bege/50 pt-3">
                                                  <h5 className="font-extrabold text-tas-dark uppercase text-[9px] tracking-wider mb-2">
                                                    Composição de Itens
                                                  </h5>
                                                  {!order.items ||
                                                  order.items.length === 0 ? (
                                                    <p className="text-[10px] text-gray-400 italic">
                                                      Nenhum item adicionado a
                                                      este pedido simulado.
                                                    </p>
                                                  ) : (
                                                    <div className="space-y-2 bg-white p-2.5 rounded-xl border border-tas-bege/40">
                                                      {order.items.map(
                                                        (
                                                          ...
                                                          <div
                                                            key={itemIdx}
                                                            className="flex justify-between items-center text-[11px] py-1 border-b border-gray-50 last:border-0"
                                                          >
                                                            <div>
                                                              <span className="font-bold text-tas-dark">
                                                                {item.product
                                                            ...
                                                            <span className="font-mono font-bold text-tas-dark">
                                                              R${" "}
                                                              {Number(
                                                                item.totalPrice,
                                                              ).toFixed(2)}
                            ...
                            {showAdminTab === "json_io" && (
                              <div className="space-y-4">
                                <div className="bg-white p-4 rounded-3xl border border-tas-bege space-y-3.5 text-xs text-left">
                                  <h3 className="font-serif font-black text-tas-dark text-sm border-b pb-1">
                                    Gestão Completa via Dump JSON
                                  </h3>
                                  <p className="text-[10px] text-gray-400 leading-normal">
                                    Você pode carregar as informações
                                    corporativas copiando e colando um formato
                                    de arquivo JSON unificado estruturado. Isso
                                    atualiza instantaneamente todo o aplicativo!
                                  </p>
                                  ...
                                  <div className="flex gap-2">
                                      ...
                                      onClick={() => {
                                        try {
                                          ...
                                          if (parsed) {
                                            ...
                                            alert(
                                              "Parabéns! Banco de Dados reinjetado e sincronizado com total sucesso! 📊🚀",
                                            );
                                          }
                                        } catch (error) {
                                          alert(
                                            "Formato JSON inválido - verifique chaves e vírgulas!",
                                          );
                                        }
                                      }}
                                    ...
                                    >
                                      <Upload className="h-4 w-4" /> Importar e
                                      Aplicar
                                    </button>
                                    ...
                                    <button
                                      onClick={() => {
                                        ...
                                        alert(
                                          "Código de dados copiado para sua área de transferência! Guarde isso como backup da sua empresa. 📝✅",
                                        );
                                      }}
                  ...
                  <footer className="sticky bottom-0 bg-white/95 backdrop-blur-md border-t border-tas-bege px-3.5 py-2 flex items-center justify-around z-30 shrink-0 select-none">
                    ...
                    >
                      ...
                      {currentView === ActiveView.HOME && (
                        <span className="absolute -bottom-1 h-1.5 w-1.5 bg-tas-gold rounded-full"></span>
                      )}
                    </button>
                    ...
                    <button
                      onClick={() => {
                        setSelectedProduct(null);
                        setSelectedArticle(null);
                        setCurrentView(ActiveView.RECIPES);
                      }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        setSelectedArticle(null);
                        setCheckoutStep("cart");
                        setCurrentView(ActiveView.CART);
                      }}
              ...
              <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-xl flex flex-col gap-4 relative animate-slideup">
                ...
                <div className="flex gap-3 mt-2">
                  <button
                    ...
                    className="flex-1 py-3 bg-gray-100 hover:bg-gray-200 text-tas-dark font-bold rounded-xl text-xs transition-colors cursor-pointer shadow-sm"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-1 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-xs transition-colors cursor-pointer shadow-sm"
                  >
                    Confirmar
                  </button>
                </div>
